...
int			fill_menu(t_image **menu)
{
	...
	while (i < SIZE_MENU)
	{
		while (mask > 0x00000000 && i < SIZE_MENU)
		{
			while (j < SIZE_WINDOW_Y)
			{
				image_set_pixel(menu, i, j++, 0x00CCAAFF | mask);
			}
		...
		while (j < SIZE_WINDOW_Y)
			image_set_pixel(menu, i, j++, 0x00CCAAFF);
		j = 0;
		i++;
	}
	return (0);
...
{
	*image = (t_image *)malloc(sizeof(t_image));//нужно замоллочить извне?
