...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/10 13:45:59 by hchau             #+#    #+#             */
/*   Updated: 2020/02/10 13:46:07 by hchau            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../incs/header.h"

void	write_rest_menu(t_window *ptr)
{
...
void	current_menu(t_window *ptr, t_full_image *full)
{
	if (full->patrick.on)
	{
	...
	mlx_string_put(ptr->mlx, ptr->win, 770, 160, 0x00BBBBFF, "scroll up/down");
	mlx_string_put(ptr->mlx, ptr->win, 770, 480, 0x00BBBBFF, "1, 2, 3, 4, 5");
	mlx_string_put(ptr->mlx, ptr->win, 770, 720,
			0x00BBBBFF, "(only in meme mode):");
	mlx_string_put(ptr->mlx, ptr->win, 770, 740, 0x00BBBBFF, "press g");
	mlx_string_put(ptr->mlx, ptr->win, 770, 820, 0x00BBBBFF, "ESC");
...
void	print_menu(t_draw_menu *menu, t_window *ptr, t_full_image *full)
{
	...
	current_menu(ptr, full);
	mlx_string_put(ptr->mlx, ptr->win, 770, 200,
				0x00BBBBFF, "rescale fractal:");
	mlx_string_put(ptr->mlx, ptr->win, 770, 520, 0x00BBBBFF, "move:");
	...
	mlx_string_put(ptr->mlx, ptr->win, 770, 600, 0x00BBBBFF, "space");
...
void	draw(t_full_image *full)
{
	...
	if (full->patrick.on && full->fractal.color_shift ==
							LAST_COLOR && !full->patrick.depr)
